 ...
 * @version	: 2.0
 * @date 	: 2024-07-27
 * @note 	: It encapsulates the functionalities needed to control the keys and can be extended for
 * *
 * *
 * ******* 1. Improvement *********
...
{
    key_device_t *p_dev_key_ctrl = key_device_get_pointer(KEY_ID_CTRL);

    // if key is pressed,return the key index
    if (KEY_STATUS_CLICK == p_dev_key_ctrl->params.status)
    {
